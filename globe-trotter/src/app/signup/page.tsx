...
export default function SignUpPage() {
    const { signInWithGoogle, signUpWithEmail, user, error: authError } = useAuth();
    ...
    return (
        <div className="min-h-screen flex items-center justify-center relative overflow-hidden bg-[#0a0c10] font-inter">
            ...
            <main className="relative z-10 w-full max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center min-h-[85vh]">
                {/* Left Side: Hero Text */}
                <motion.div
                ...
                >
                    <div className="space-y-4">
                        <span className="text-white/60 tracking-[0.8em] text-xs font-bold uppercase">Journey</span>
                    ...
                    <div className="max-w-xl space-y-6">
                        <p className="text-2xl font-bold text-white tracking-tight leading-snug">
                            Join the global community of travelers.
                        </p>
                        <p className="text-white/50 text-xl font-medium leading-relaxed">
                            Create an account to start planning your next sequence.
                            Synchronize your travel DNA with global possibilities.
                        </p>
                    </div>
                </motion.div>
                    ...
                    <div className="w-full max-w-[520px] bg-[#0c121d]/90 backdrop-blur-3xl p-10 md:p-14 rounded-[2.5rem] border border-white/5 shadow-[0_40px_100px_rgba(0,0,0,0.5)]">
                        ...
                        <form onSubmit={handleEmailSignUp} className="space-y-5">
                            <div className="space-y-3">
                                <label className="text-sm font-bold text-white/80 ml-1 uppercase tracking-widest text-[10px]">Full Name</label>
                                <input
                                    type="text"
                                    placeholder="Enter your name"
                                    value={name}
                                    onChange={(e) => setName(e.target.value)}
                                    className="w-full bg-[#1c2331] border border-white/10 rounded-2xl py-4.5 px-8 text-white outline-none focus:border-blue-500 transition-all placeholder:text-gray-500 font-medium"
                                    required
                                />
                            </div>
                            ...
                            <button
                                type="submit"
                                disabled={isLoading}
                                className="w-full h-18 bg-[#4285f4] hover:bg-[#3b77db] text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] transition-all active:scale-[0.98] shadow-[0_15px_40px_rgba(66,133,244,0.4)] flex items-center justify-center gap-3 disabled:opacity-50 mt-4"
                            >
                        ...
                        >
                            ...
                            <span className="flex-1 mr-10 font-medium">Join with Google</span>
                        </button>

                        <div className="mt-8 text-center">
                            <p className="text-xs text-white/40 font-bold uppercase tracking-widest">
                                Already have an account? <Link href="/login" className="text-white hover:underline ml-2">Sign In</Link>
                            </p>
                        </div>
                        ...
                        <AnimatePresence>
                            {displayError && (
                                <motion.div
                                    ...
                                    className="mt-6 p-5 rounded-2xl bg-red-500/10 border border-red-500/20 flex items-center gap-4"
                                >
